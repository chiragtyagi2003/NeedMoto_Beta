...
class AdminMainController extends GetxController {
  ...
  void fetchVehicleData() async {
    try {
      ...
      QuerySnapshot<Map<String, dynamic>> snapshot = await FirebaseFirestore.instance.collection('vehicles').get();
  ...
  Future<void> countAndUpdateVariables() async {
    try {
      ...
      QuerySnapshot bookedVehiclesSnapshot = await vehiclesCollection.where('onRide', isEqualTo: true).get();
      int booked = bookedVehiclesSnapshot.size;

      // Get the documents where 'onRide' field is false
      QuerySnapshot availableVehiclesSnapshot = await vehiclesCollection.where('onRide', isEqualTo: false).get();
